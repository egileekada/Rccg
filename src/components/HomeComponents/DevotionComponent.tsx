...
import DevotionModal from '../../modals/DevotionModal';

export default function DevotionComponent() {
    ...
    return(
        <div className='w-full pb-10' >
            <div className='w-full flex items-center' >
                <div className='' >
                    <p style={{color: '#121212'}} className='font-Poppins-Bold text-2xl' >Manage Devotions</p>
                    <p style={{color: '#727272'}} className='font-Poppins-Regular mt-1 text-sm' >Manage all exiting devotion and upload new devotions</p>
                </div>
            ...
            : 
                <div className='mt-6 w-full grid grid-cols-4 gap-6' >
                    {data.map((item: any)=> {
                        return(
                            <div key={item._id} style={{backgroundColor: '#F4F4F4'}} className='py-4 px-6 rounded-md relative' >
                                ...
                                <p style={{color: '#727272'}} className='text-sm mt-4 font-Poppins-Regular' >{DateFormat(item.date)}</p>
                            </div>
                        )
                    })}
                </div>
            ...
            {showModal ? 
                (
                    <>
                        <div className="h-auto flex justify-center items-center overflow-x-hidden overflow-y-hidden fixed pb-4 px-4 inset-0 z-50 outline-none focus:outline-none"> 
                            <DevotionModal value={value} close={setShowModal} />
                        </div> 
                        <div className="opacity-50 fixed flex flex-1 inset-0 z-40 bg-black"/>
                    </>
                ) : null} 
