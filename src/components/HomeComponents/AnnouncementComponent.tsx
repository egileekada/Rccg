...
export default function AnnouncementComponent() {
    ...
    const DeleteHandler =()=>{
        fetch(`https://rccg-web-api.herokuapp.com/announcements/${index}`, {
    ...
    return(
        <div className='w-full pb-10' >
            <div className='w-full flex items-center' >
                <div className='' >
                    <p style={{color: '#121212'}} className='font-Poppins-Bold text-2xl' >Manage Announcement</p>
                    <p style={{color: '#727272'}} className='font-Poppins-Regular mt-1 text-sm' >Manage all exiting announcement and make new ones</p>
                </div>
                <button onClick={()=> OpenModal()} style={{border:'1px solid #28166F'}} className='w-36 text-active text-sm py-3 ml-auto font-Poppins-Regular justify-center rounded-md flex items-center' >
            ...
            : 
                <div className='mt-6 w-full grid grid-cols-4 gap-6' >
                    {data.map((item: any) => {
                        return(
                            <div key={item._id} style={{backgroundColor: '#F4F4F4'}} className='py-4 px-6 pb-10 rounded-md relative' >
                                ...
                                :null}
                                <p style={{color: '#727272'}} className='text-xs font-Poppins-Regular mb-auto flex flex-1 h-full' >{item.announcement}</p>
                                
                                <div className='w-full flex items-center mt-1 ' >
                                    <p style={{color: '#28166F'}} className='text-sm font-Poppins-Regular' >{item.name}</p>
            ...
            {showModal ? 
                (
                    <>
                        <div className="h-auto flex justify-center items-center overflow-x-hidden overflow-y-hidden fixed pb-4 px-4 inset-0 z-50 outline-none focus:outline-none"> 
                            <AnnouncementModal value={value} close={setShowModal} />
                        </div> 
                        <div className="opacity-50 fixed flex flex-1 inset-0 z-40 bg-black"/>
                    </>
                ) : null} 
